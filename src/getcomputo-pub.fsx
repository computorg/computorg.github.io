        ...
        if not m.Success then
            ...
            for line in fm do
                ...
                if trimmed.StartsWith("#") || trimmed = "" then
                    ()
                else if trimmed.StartsWith("-") && currentKey <> "" then
    ...
    | Some elem when elem.ValueKind = JsonValueKind.Array ->
        let authors =
            ...
            |> Seq.choose (fun authorElem ->
                ...
                | _ ->
                    if authorElem.ValueKind = JsonValueKind.String then
                        Some(authorElem.GetString())
                    else
                        None)
            |> Seq.toList
                    ...
                    | None ->
                        ...
                        let fileMetadata =
                            ...
                            |> Seq.choose (fun f ->
                                ...
                                | Some md when tryGetMetadataFromElement md |> Option.isSome ->
                                    Some(getJsonString f "path", md)
                        ...
                        | None ->
                            ...
                            match
                                getJsonObject quartoJson "config"
                                |> Option.bind (fun c -> getJsonObject c "metadata")
                            with
                            ...
                            | _ ->
                                let sampleInfo =
                                    ...
                                    |> Option.bind (fun fi ->
                                        fi.EnumerateObject() |> Seq.tryHead |> Option.map (fun p -> p.Value))
                                ...
                                Error
                                    $"No metadata found. Available keys: {keys}. Sample fileInformation entry: {sampleInfo}"
                | None ->
                    let keys =
                        quartoJson.EnumerateObject() |> Seq.map (fun p -> p.Name) |> String.concat ", "
...
let runQuartoInspect (repoPath: string) : Task<Result<JsonElement, string>> =
    task {
        try
            ...
            let outputFile =
                System.IO.Path.Combine(tempDir, $"quarto-inspect-{System.Guid.NewGuid()}.json")
            ...
            else if System.IO.File.Exists(outputFile) then
...
let getPublishedRepoContent
    (repo: Repository)
    : Task<Result<JsonElement * Repository * Map<string, string> option, string>> =
    task {
        try
            let startTime = System.DateTime.Now
            ...
            let tempDir =
                System.IO.Path.Combine(System.IO.Path.GetTempPath(), $"{repo.Name}-{uniqueId}")
            ...
            if quartoFiles.IsEmpty then
                ...
                printfn
                    "    [%s] Found %d Quarto files, downloading..."
                ...
                for path in quartoFiles do
                    try
                        let! rawBytes =
                            client.Repository.Content.GetRawContent(repo.Owner.Login, repo.Name, path)
                            |> Async.AwaitTask
                        ...
                        if localPath.EndsWith(".qmd") && firstQmdPath.IsNone then
                            firstQmdPath <- Some localPath

                        downloadedCount <- downloadedCount + 1
                    with ex ->
                        printfn
                            "    [%s] Warning: Could not download %s: %s"
                            (System.DateTime.Now.ToString("HH:mm:ss"))
                            path
                            ex.Message
                ...
                printfn
                    "    [%s] Downloaded %d files (%A elapsed)"
                    (System.DateTime.Now.ToString("HH:mm:ss"))
                    downloadedCount
                    downloadElapsed
                ...
                if downloadedCount = 0 then
                    ...
                    | Ok json ->
                        ...
                        printfn
                            "    [%s] ✓ Successfully inspected (total: %A)"
                            (System.DateTime.Now.ToString("HH:mm:ss"))
                            elapsed
        ...
        try
            ...
            | Ok metadata ->
                ...
                let repoName =
                    ...
                    if System.String.IsNullOrWhiteSpace(fromMeta) then
                        repo.Name
                    else
                        fromMeta
                ...
                let draft =
                    ...
                    if System.String.IsNullOrWhiteSpace(raw) then
                        "false"
                    elif raw.Equals("true", System.StringComparison.OrdinalIgnoreCase) then
                        "true"
                    elif raw.Equals("false", System.StringComparison.OrdinalIgnoreCase) then
                        "false"
                    else
                ...
                let journal =
                    citationObj
                    |> Option.bind (fun c -> getJsonObject c "container-title")
                    |> Option.map (fun j -> j.GetString())
                    |> Option.defaultValue ""

                let doi =
                    citationObj
                    |> Option.bind (fun c -> getJsonObject c "doi")
                    |> Option.map (fun d -> d.GetString())
                    |> Option.defaultValue ""

                let pdfUrl =
                    citationObj
                    |> Option.bind (fun c -> getJsonObject c "pdf-url")
                ...
                let bibtex =
                    if bibtexFromMeta <> "" then
                        bibtexFromMeta
                    else
                        getBibTeXFromRepo repo
            ...
            | Error _ ->
                ...
                | Some fm ->
                    let tryGet key =
                        fm |> Map.tryFind key |> Option.defaultValue ""
...
|> serializeToYaml
|> (fun n ->
    let path = Path.Combine(repoRoot, "site", "published.yml")
    File.WriteAllText(path, n)
    printfn "  ✓ Wrote %s (%d published papers)" path publishedCount)

let pipelinePath = Path.Combine(repoRoot, "site", "pipeline.yml")
...
let rssContent =
    ...
    for item in items do
        ...
        let titleWithAuthors =
            if System.String.IsNullOrWhiteSpace(item.authors) then
                item.title
            else
...
let rssPath = Path.Combine(repoRoot, "site", "published.xml")
...
|> serializeToYaml
|> (fun n ->
    let path = Path.Combine(repoRoot, "site", "mock-papers.yml")
    File.WriteAllText(path, n)
